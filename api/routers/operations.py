...
async def create_implementation(implementation: ImplementationCreate):
    ...
    try:
        ...
        if not db['concepts'].find_one({"name": implementation.concept}):
            raise HTTPException(status_code=400, detail="Referenced concept does not exist")
        ...
        created_implementation = db['implementations'].find_one({"_id": result.inserted_id})
    ...
    try:
        ...
        for concept in tool.concepts:
            if not db['concepts'].find_one({"name": concept}):
                raise HTTPException(status_code=400, detail=f"Referenced concept does not exist: {concept}")

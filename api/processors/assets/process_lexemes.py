...
class ProcessLexemes(BaseAssetProcessor):
    ...
    async def process_asset(self, file_hash: str, asset: dict, db: dict, span):
        ...
        try:
            ...
            logger.info(f"Processing {len(prompts_to_run)} prompts for category {category}")
            ...
            for prompt_file in prompts_to_run:
                try:
                    ...
                    if "error" in response:
                        error_msg = f"Chat API error for {prompt_file}: {response['error']}"
                    ...
                    if prompt_lexemes:
                        ...
                        logger.info(f"Extracted {len(prompt_lexemes)} lexemes from {prompt_file}")
                    else:
                        logger.warning(f"No lexemes found in response for {prompt_file}")

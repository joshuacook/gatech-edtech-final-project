    ...
    try:
        ...
        if processor_type == "tables":
            ...
            if "processed_paths" in asset:
                print(
                    "Processed paths:", json.dumps(asset["processed_paths"], indent=2)
                )
            ...
            if "splitting" in result:
                print(
                    ...
                    json.dumps(result["splitting"], indent=2),
                )
        ...
        span.event(name=f"{processor_type}_completed", metadata={"result": result})
        ...
        print(f"\nERROR: {error_msg}")
        span.event(
            name=f"{processor_type}_error", metadata={"error": str(e)}, level="error"
        )
        raise
    finally:
        span.end()
